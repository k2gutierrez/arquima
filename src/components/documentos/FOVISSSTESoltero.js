...
export default function FOVISSSTESoltero(props) {
  ...
  const handleExpediente = async () => {
    if (currentfile == null) return;
    try {
      const Ref = ref(storage, `${props.id}/Expediente_electronico_Cliente_${currentfile.name}`)
      const carg = await uploadBytes(Ref, currentfile)
      
      const enlaceUrl = await getDownloadURL(carg.ref)
      const docRef = doc(db, 'clientes', props.id)
      await updateDoc(docRef, {
        ExpedienteElectronicoCliente: {
          url: enlaceUrl,
          vendedor: true
        },
        historial: arrayUnion({
          registrado: props.currentUser,
          fecha: Timestamp.fromDate(new Date()),
          comentario: "Se subió el expediente electrónico único del cliente"
        })
      })
      setShow(true)
  ...
  return (
    <>
      ...
      <div className='row text-start mx-5 px-5'>
          ...
          <form >
            <div className="mb-3">
              <label for="actaMatrimonioCliente" className="form-label">Últimos 3 talones de pago (subir un archivo con los 3)</label>
          ...
          <form >
            <div className="mb-3">
              <label for="expediente" className="form-label">Expediente electrónico único</label>
              <input className="form-control" onChange={(e) => setCurrentfile(e.target.files[0])} type="file" id="expediente" />
              <button type='button' onClick={handleExpediente} className='btn btn-secondary my-3' >Subir archivo</button>
            </div>
          </form>
